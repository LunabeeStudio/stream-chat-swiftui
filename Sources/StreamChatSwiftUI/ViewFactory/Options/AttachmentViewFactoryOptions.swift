...
public final class CustomAttachmentViewTypeOptions: Sendable {
    ...
    /// Binding to the currently scrolled message ID.
    public let scrolledId: Binding<String?>
    /// The slot in which the custom attachment is displayed.
    /// - Note: Kept from the fork's custom-attachment-layout feature. Currently inert — the
    ///   message view renders custom attachments in a single slot, so this is always `.all`.
    ///   See `MessageView` for the previous top/bottom usage (commented out there).
    public let layout: CustomAttachmentLayout
    ...
    public init(
        ...
        availableWidth: CGFloat,
        scrolledId: Binding<String?>,
        layout: CustomAttachmentLayout = .all
    ) {
        self.message = message
        self.isFirst = isFirst
        self.availableWidth = availableWidth
