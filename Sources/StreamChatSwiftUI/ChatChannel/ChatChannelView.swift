...
public struct ChatChannelView<Factory: ViewFactory>: View, KeyboardReadable {
    ...
    public var body: some View {
        ...
        .onPreferenceChange(FloatingComposerHeightPreferenceKey.self) { value in
            ...
            // Reserve the bottom safe area so the last message clears the floating composer.
    ...
    ///
    /// Any minimum gap on devices with no safe area is owned by the composer content itself
    /// (e.g. `MessageComposerView`), so a flush composer (e.g. a locked one) simply omits it.
