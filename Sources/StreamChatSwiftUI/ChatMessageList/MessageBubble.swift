...
/// - Note: Changes from original implementation:
///   - Add showBubble parameter that doesn't add the bubble background modifier if false
public struct MessageBubbleModifier: ViewModifier {
    ...
    // - Note: The fork keeps only the `showBubble` gate: when false the content is rendered
    //   without any bubble background. The bubble styling itself now follows v5.
    //   Previous fork behavior (replaced by v5's design system):
    //     - borderColor: .clear (no border)        — now `message.bubbleBorder(colors:)` (themed)
    //     - cornerRadius: 12 (fixed)               — now `cornerRadius ?? tokens.messageBubbleRadiusGroupBottom`
    //     - bubbleCorners without `layoutDirection` — now mirrors corners for RTL
