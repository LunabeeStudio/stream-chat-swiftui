...
public struct ReactionsOverlayContainer: View {
    ...
    public var body: some View {
        ...
        // - Note: Fork addition. v5 removed this manual offset (it positions the overlay itself).
        //   Kept because the app uses `customHorizontalOffset` to nudge reactions on some messages.
        //   `messageOffsetX` falls back to `message.reactionOffsetX(...)` when no custom offset is set.
        .offset(
            x: messageOffsetX,
            y: -20
        )
...
// - Note: Fork addition. v5 removed this helper (it positions the reactions overlay itself).
//   Kept because `ReactionsOverlayContainer` still applies a manual `.offset` driven by it.
