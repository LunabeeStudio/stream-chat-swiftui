...
/// - Note: Now follows v5's link preview (image + title + description, themed via design tokens
///   and `makeMessageAttachmentItemViewModifier`). The fork previously rendered an image-only
///   preview: a `VStack(spacing: 8)` with an 8pt top padding, no title/description, and no item
///   modifier (the bubble was owned by the parent `MessageView`).
...
/// - Note: Now follows v5 (StreamAsyncImage, title + description shown). The fork previously
///   rendered image-only: a Nuke `LazyImage` with a gray placeholder, framed `width × (width/2)`
///   with `cornerRadius(12)`, and removed the title/description.
