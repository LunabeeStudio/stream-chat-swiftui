...
- `ChatChannel/ChatChannelView.swift` — **floating composer & bottom safe area**. Upstream only
  ...
  - **Min composer bottom inset** — devices with **no bottom safe area** (iPhone SE / Touch-ID) must not leave the composer flush at the physical bottom. The gap is owned by the **composer content**, not the channel slot, so each composer decides independently:
    ...
    - `ChatComposer/MessageComposerView.swift` adds its own `minBottomInset` (`bottomSafeArea == 0 && !keyboardShown ? 8 : 0`) as the outermost `.padding(.bottom, …)`. So the interactive composer keeps its 8pt gap on SE, while a flush composer (e.g. the app's locked-chat composer returned from `makeMessageComposerViewType`) simply omits it and sits flush.
    - On devices with a home indicator this is a no-op (safe area handles it). Not a v4 regression — upstream/v4 also left the composer flush on SE — this is a new fix.
